...
def backward(net: TM, conf_eval_losses, final_classifier_losses, q_m_losses, a_m_losses):
    ...
    total_conf_eval_loss = sum(loss.mean() for loss in conf_eval_losses)
    ...
    total_final_classifier_loss = sum(fc_loss[-1] for fc_loss in final_classifier_losses)
    ...
    total_q_m_loss = sum(loss.mean() for loss in q_m_losses)
    total_a_m_loss = sum(loss.mean() for loss in a_m_losses)
